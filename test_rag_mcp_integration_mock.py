#!/usr/bin/env python3
"""
Мок-тестирование интеллектуальной маршрутизации между RAG и MCP.
Использует моки для избежания внешних зависимостей.
"""
...
async def test_rag_mcp_routing_mock():
    """Тестирует интеллектуальную маршрутизацию с моками."""
    
    print("🧪 МОК-ТЕСТИРОВАНИЕ ИНТЕЛЛЕКТУАЛЬНОЙ МАРШРУТИЗАЦИИ RAG/MCP")
    print("=" * 60)
    
    # Мокаем внешние зависимости
    with patch('route_planner.agent.AsyncOpenAI') as mock_openai_class:
        mock_openai = AsyncMock()
        mock_openai_class.return_value = mock_openai
        
        # Мокаем ответы LLM
        mock_message = Mock()
        mock_message.content = "Тестовый ответ"
        mock_choice = Mock()
        ...
        # Создаем агента с моком RAG
        agent = RoutePlannerAgent(model="openrouter/free")
        
        # Мокаем RAG retriever
        agent.rag_retriever = Mock()
        agent.rag_retriever.search = Mock(return_value=[])
        
        # Мокаем MCP оркестратор
        agent.orchestrator.is_available = Mock(return_value=False)
...
def test_rag_functionality_mock():
    """Тестирует функциональность RAG retriever с моками."""
    print("\n🧪 МОК-ТЕСТИРОВАНИЕ RAG ФУНКЦИОНАЛЬНОСТИ")
    print("=" * 40)
...
async def main():
    ...
    print("🚀 ЗАПУСК МОК-ТЕСТИРОВАНИЯ ИНТЕГРАЦИИ RAG/MCP")
    print("=" * 60)
    
    # Тестируем RAG функциональность
    rag_ok = test_rag_functionality_mock()
    
    if rag_ok:
        # Тестируем интеллектуальную маршрутизацию
        routing_results = await test_rag_mcp_routing_mock()
        
        # Сохраняем результаты
        import json
        with open("rag_mcp_integration_mock_results.json", "w", encoding="utf-8") as f:
            json.dump(routing_results, f, ensure_ascii=False, indent=2)
        
        print(f"\n✅ Результаты сохранены в rag_mcp_integration_mock_results.json")
        
        # Проверяем успешность тестирования
        success_rate = sum(1 for r in routing_results if r['match']) / len(routing_results)
        
        if success_rate >= 0.7:
            print("🎉 МОК-ТЕСТИРОВАНИЕ ПРОЙДЕНО УСПЕШНО!")
            print(f"   Успешность: {success_rate*100:.1f}%")
        else:
            print("⚠️ МОК-ТЕСТИРОВАНИЕ ТРЕБУЕТ ДОРАБОТКИ")
            print(f"   Успешность: {success_rate*100:.1f}%")
    else:
        print("❌ МОК-ТЕСТИРОВАНИЕ ПРЕРВАНО: RAG не работает")
